    ...
    <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-2">
        ...
        @if($user && $user->role === 'lecturer')
            <div class="d-block d-md-none">
                <a href="{{ route('classes.create') }}" class="btn btn-primary rounded-circle d-flex align-items-center justify-content-center shadow-sm" style="width: 40px; height: 40px;" title="Buat Kelas">
    ...
    @if($classes->count() > 0)
        <div class="row row-cols-1 row-cols-sm-2 row-cols-lg-3 row-cols-xl-4 g-3 g-md-4">
            @foreach($classes as $class)
                <div class="col">
                    <div class="card h-100 gc-card border shadow-sm position-relative" style="border-radius: 8px; overflow: hidden; transition: box-shadow 0.2s ease-in-out;">
                        
                        @php
                            $bgColors = ['#1a73e8', '#1e8e3e', '#f9ab00', '#d93025', '#8e24aa', '#00acc1'];
                            $currentBg = $bgColors[$loop->index % count($bgColors)];
                        @endphp
                        <div class="p-3 text-white position-relative d-flex flex-column justify-content-between" style="background-color: {{ $currentBg }}; height: 100px;">
                            <div class="pe-5">
                                <a href="{{ route('classes.show', $class) }}" class="text-white text-decoration-none fw-medium fs-5 lh-sm d-block text-truncate gc-card-title" title="{{ $class->name }}">
                                    {{ $class->name }}
                                </a>
                            ...
                            <small class="text-white-50 text-truncate" style="font-size: 0.8rem; font-weight: 400; max-width: 75%;">
                        ...
                        <div class="card-body p-3 d-flex flex-column justify-content-between" style="min-height: 120px; background-color: #ffffff;">
                            <div class="text-muted small">
                                @if($class->schedule_day)
                                    @php
                                        $schedules = is_array($class->schedule_day) ? $class->schedule_day : json_decode($class->schedule_day, true);
                                    @endphp
                                    @if(is_array($schedules) && count($schedules) > 0)
                                        <div class="mb-1 text-truncate">
                                            ...
                                            @foreach($schedules as $day => $time)
                                                <span class="text-capitalize text-dark fw-medium">{{ $day }}</span> ({{ $time }}){{ !$loop->last ? ',' : '' }}
                                            @endforeach
                                        </div>
                                    @endif
                                @endif
                        ...
                        <div class="card-footer bg-white border-top-0 px-3 py-2 d-flex justify-content-end gap-1">
                            <a href="{{ route('classes.show', $class) }}#tasks" class="btn btn-link text-secondary p-2 rounded-circle border-0 d-flex align-items-center justify-content-center" style="width: 36px; height: 36px;" title="Buka Tugas Kuliah">
    ...
    @else
        <div class="d-flex flex-column align-items-center justify-content-center text-center my-5 py-5">
            ...
            <p class="text-secondary mx-auto mb-4 small" style="max-width: 360px;">
                ...
                @else
                    Mintalah kode akses kelas (`join_code`) dari dosen pengampu Anda untuk bergabung ke dalam perkuliahan.
                @endif
            </p>
            @if($user && $user->role === 'lecturer')
                <a href="{{ route('classes.create') }}" class="btn btn-primary fw-medium px-4 py-2" style="border-radius: 4px; font-size: 0.9rem;">
                    <i class="fas fa-plus me-2"></i>Buat Kelas Pertama
                </a>
            @else
                <button type="button" class="btn btn-outline-primary fw-medium px-4 py-2" style="border-radius: 4px; font-size: 0.9rem;" data-bs-toggle="modal" data-bs-target="#joinModal">
