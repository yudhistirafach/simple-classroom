...
<div class="container-fluid px-3 px-md-4 py-3">
        ...
        <div class="card-body position-relative d-flex flex-column justify-content-end p-4 p-md-5 z-3" style="z-index: 2;">
            ...
            <p class="text-white-50 mb-0" style="font-size: 0.95rem;">
                @if(auth()->user() && auth()->user()->role === 'lecturer')
                    Pantau ringkasan kelas dan tugas yang sedang Anda kelola.
                @else
    ...
    <div class="alert bg-white border shadow-sm d-flex align-items-center mb-4 py-3" style="border-radius: 8px; border-left: 4px solid #1a73e8 !important;" role="alert">
        ...
        <div class="text-secondary" style="font-size: 0.9rem;">
            @if(auth()->user() && auth()->user()->role === 'lecturer')
                Halaman ini adalah preview singkat. Akses menu <strong>Kelas Saya</strong> pada <i>sidebar</i> untuk mengelola ruang kelas secara rinci.
            @else
        ...
        <div class="col-12 col-md-4">
            <div class="card h-100 border-0 shadow-sm gc-stat-card" style="border-radius: 8px;">
                <div class="card-body p-4 d-flex align-items-center">
                    ...
                    <div>
                        <p class="text-secondary mb-0 fw-medium" style="font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.5px;">
                            {{ auth()->user()->role === 'lecturer' ? 'Total Kelas' : 'Kelas Diikuti' }}
                        </p>
                        <h2 class="mb-0 fw-bold text-dark lh-1 mt-1">{{ $totalClasses }}</h2>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-12 col-md-4">
            <div class="card h-100 border-0 shadow-sm gc-stat-card" style="border-radius: 8px;">
                <div class="card-body p-4 d-flex align-items-center">
                    ...
                    <div>
                        <p class="text-secondary mb-0 fw-medium" style="font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.5px;">
                            {{ auth()->user()->role === 'lecturer' ? 'Tugas Berjalan' : 'Tugas Belum Selesai' }}
                        </p>
                        <h2 class="mb-0 fw-bold text-dark lh-1 mt-1">{{ $totalActiveTasks }}</h2>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card h-100 border-0 shadow-sm gc-stat-card" style="border-radius: 8px;">
                <div class="card-body p-4 d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <h2 class="mb-0 fw-bold text-dark lh-1 mt-1">{{ $totalAnnouncements }}</h2>
                    </div>
                </div>
            </div>
        </div>
